...
class ETCore{
	...
	*/
	...
	function ParseCode(
		...
		if (eregi("run:",$ShowTPL)){
			$run	 = 1;
			//Fix =
			$ShowTPL = preg_replace_callback('/(\{|<!--\s*)run:(\}|\s*-->)\s*=/','{run:}echo ',$ShowTPL);
			$ShowTPL = preg_replace_callback('/(\{|<!--\s*)run:\s*=/','{run:echo ',$ShowTPL);
			//Fix Run 1
			$ShowTPL = preg_replace_callback('/(\{|<!--\s*)run:(\}|\s*-->)\s*(.+?)\s*(\{|<!--\s*)\/run(\}|\s*-->)/is', '(T_T)\\3;(T_T!)',$ShowTPL);
		}
		
		//Fix XML
		if (eregi("<?xml",$ShowTPL)){
			$ShowTPL = @preg_replace_callback('/<\?(xml.+?)\?>/is', '<ET>\\1</ET>', $ShowTPL);
		}
		...
		//编译运算
 		$ShowTPL = @preg_replace_callback($this->Compile, $this->Analysis, $ShowTPL);
		...
		//分析图片地址
 		...
 		$ShowTPL = str_replace('$','\$',$ShowTPL);
			...
			//修复php运行错误
			$ShowTPL = @preg_replace_callback("/\";(.+?)echo\"/e", '$this->FixPHP(\'\\1\')', $ShowTPL);
			
			//Fix Run 2
			if ($run==1){
				$ShowTPL = preg_replace_callback("/\(T_T\)(.+?)\(T_T!\)/ise", '$this->FixPHP(\'\\1\')', $ShowTPL);
			}
			
			//还原xml
			$ShowTPL = (strrpos($ShowTPL,'<ET>'))?@preg_replace_callback('/ET>(.+?)<\/ET/is', '?\\1?', $ShowTPL):$ShowTPL;
	...
	*/
	function inc_preg(
		...
		return preg_replace_callback('/<\!--\s*\#include\s*file\s*=(\"|\')([a-zA-Z0-9_\.\|]{1,100})(\"|\')\s*-->/eis', '$this->inc("\\2")', preg_replace_callback('/(\{\s*|<!--\s*)inc\:([^\{\} ]{1,100})(\s*\}|\s*-->)/eis', '$this->inc("\\2")', $content));
	}
...
<!-- END -->";
	...
	*/
	function inc_php(
		...
		foreach((array)explode('&',$parse['query']) AS $vals){
			$code_array .= preg_replace_callback('/(.+)=(.+)/',"\$_GET['\\1']= \$\\1 ='\\2';",$vals);
		}
	...
	*/
	function Color(
		...
		if($color != ''){
			$OutStr = preg_replace_callback("/(.+),(.+)/","_i%2===0)?'\\1':'\\2';",$color);
			if(strrpos($OutStr,"%2")){
	...
	function inc_list(){
		if(is_array($this->IncList)){
			...
			if(strrpos($EXTS[$Last],'Ease_Templatepage=Clear') && trim($EXTS[$Last]) != ''){
				...
				$GoURL = urldecode(preg_replace_callback("/.+?REFERER=(.+?)!!!/","\\1",$EXTS[$Last]));
			...
			//连接地址
			$BackURL = preg_replace_callback("/.+\//","\\1",$this->Server['REQUEST_URI']);
